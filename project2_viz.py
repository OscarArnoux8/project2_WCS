...
st.write(f'''
    <a target="_self" href="https://share.streamlit.io/pilouliz/movie_reco/main/app.py" style="text-decoration: none">
...
with row_1_1, _lock:
  ...
  ax1 = imdb.groupby(imdb.startYear)['numVotes'].mean().plot(color="#1e90ff")
  plt.xlabel('Date')
  plt.ylabel('Votes')
  st.pyplot(fig1)
...
with row_1_2, _lock:
  ...
  ax2 = imdb.groupby((imdb['startYear']//10)*10)['tconst'].count().plot(kind='barh',color="#1e90ff")
  plt.ylabel('Décennie')
